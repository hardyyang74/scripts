...
#define CFG_PATH "/tmp/hotpoint"
#define CNTR_CODE "CN"
static const char DNSMASQ_CONF_DIR[] = CFG_PATH"/dnsmasq.conf";
// hostapd
static const char HOSTAPD_CONF_DIR[] = CFG_PATH"/hostapd.conf";
static const char HOSTAPD_CHANNEL[] = CFG_PATH"/hotchannel";
...
int create_hostapd_file(const char* name, const char* password)
{
    ...
    if (fp != 0) {
        ...
        fpChan = fopen(HOSTAPD_CHANNEL, "r");
        if (NULL != fpChan) {
            char buf[32];

            fscanf(fpChan, "channel=%d", &best_chan);
            fclose(fpChan);
            if (best_chan >= 1 && best_chan <= 13 ) {
...
bool creat_dnsmasq_file()
{
    ...
    if (fp != 0) {
        ...
        fputs("\n", fp);
        fputs("dhcp-range=192.168.88.80,192.168.88.84\n", fp);
        fputs("port=0\n", fp);
...
//    console_run("killall udhcpc");
    ...
    if (!strncmp(wifi_type, "RTL", 3)
        ...
        if (argc >= 3) {
            wlan_accesspoint_start(apName,argv[2]);
        } else {
